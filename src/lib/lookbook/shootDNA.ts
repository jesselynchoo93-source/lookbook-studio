...
function resolveEnvironment(style: TargetStyle, goal: CampaignGoal): string {
...
function resolveLighting(style: TargetStyle, goal: CampaignGoal): string {
  if (style === "editorial" || style === "avant_garde") {
...
export function buildMasterShootDNA(input: LookbookInput): MasterShootDNA {
  return {
    ...
    environmentFamily: resolveEnvironment(input.targetStyle, input.campaignGoal),
    lightingFamily: resolveLighting(input.targetStyle, input.campaignGoal),
